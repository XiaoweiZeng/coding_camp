...
int main(int argc, char* argv[])
{
	...
	const char* login_params			=	"appid = 5c9c7e87, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	...
	*/
	...
	printf("########################################################################\n\n");
	printf("select: need update user words list？\n0:no\n1:yes\n");

	scanf("%d", &upload_on);
	if (upload_on)
	{
		printf("update user words list...\n");
		ret = upload_userwords();
		if (MSP_SUCCESS != ret)
			goto exit;	
		printf("update user words list success\n");
	}
	run_iat("wav/iflytek01.wav", session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
exit:
	printf("exit by press any key ...\n");
	getchar();
	MSPLogout(); //退出登录
